...
float4 Sun::ComputeAttenuation(float fTheta, int nTurbidity/* = 2*/)
...
// nTurbidity >= 2
{
	...
	float4 vAttenuation = float4(fTau[0], fTau[1], fTau[2], 1);
	return vAttenuation;
}
...
float4 Sun::GetColorAndIntensity()
{
	float4 result = GetColor();
	result[3] = GetIntensity();
	return result;
}

float4 Sun::GetColorWithIntensity()
{
	float4 result = GetColor() * GetIntensity();
	return result;
}
...
void Sun::Interpolate(Sun *one, Sun *two, float f)
{
	...
	float3 dir = one->GetDirection() * (1-f) + two->GetDirection() * f;
    float3 dirn(dir[0],dir[1],dir[2]);
	dirn.normalize();
